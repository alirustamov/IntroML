...
class NN:
	...
	def feed_forward(self, x):
		...
		"""
		# z = np.dot(np.array(x).T,self.W1) + self.b1
		z = np.dot(np.transpose(x),self.W1) + self.b1
		a = np.tanh(z)
		# o = np.dot(np.array(a).T,self.W2) + self.b2
		o = np.dot(np.transpose(a),self.W2) + self.b2
		p = self.softmax(o)
